      ...
      <div className="sm:h-[100px] h-16 flex items-center justify-between">
        ...
        <div
          className={`${nav && "max-lg:left-[-100%]"} ${!nav && "max-lg:start-0"
            } flex items-center gap-11 z-40 max-lg:flex-col max-lg:items-center max-lg:justify-start max-lg:pt-[108px] max-lg:fixed max-lg:w-full max-lg:h-full max-lg:top-0 max-lg:bg-white transition-all duration-300 ease-linear`}
        >
          {NAV_LINKS_LIST.map((obj, index) => (
            <a
              ...
              className={`leading-6 navLinks ${path === obj.url &&
                ...
                } text-grey relative transition duration-300 after:transition-all after:duration-300 ease-linear hover:text-dark_black ${index === 0 || index === 1
